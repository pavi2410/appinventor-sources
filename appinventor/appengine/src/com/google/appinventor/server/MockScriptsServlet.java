...
public class MockScriptsServlet extends OdeServlet {
    ...
    private static final String SDK_CODE_START = "<html><body><script>" +
            "let mockInstances={},MockComponentRegistry={register:(a,b)=>{const c=genUUID();mockInstances[c]=new b(c),postMessage(\"registerMockComponent\",[],a,c)}};window.addEventListener(\"message\",a=>{let{action:b,args:c,type:d,uuid:e}=a.data;messageInterpreter(b,c,d,e)});function messageInterpreter(a,b,c,d){switch(a){case\"getName.callback\":{console.log(\"getName.cb\",c,d,b);break}case\"getPropertyValue.callback\":{console.log(\"getPropVal.cb\",c,d,b);break}case\"onPropertyChanged\":{mockInstances[d].onPropertyChanged(b[0],b[1]);break}}}function postMessage(a,b,c,d){let e=JSON.stringify({action:a,args:b,type:c,uuid:d});window.top.postMessage(e)}function genUUID(){return\"xxxxxxxx-xxxx-4xxx-yxxx-xxxxxxxxxxxx\".replace(/[xy]/g,function(a){var b=0|16*Math.random(),c=\"x\"==a?b:8|3&b;return c.toString(16)})}class MockVisibleExtension{constructor(a,b){this.type=a,this.uuid=b}initComponent(a){this.$el=a}getName(){postMessage(\"getName\",[],this.type,this.uuid)}getPropertyValue(a){postMessage(\"getPropertyValue\",[a],this.type,this.uuid)}changeProperty(a,b){postMessage(\"changeProperty\",[a,b],this.type,this.uuid)}refresh(a){a=a||this.$el,postMessage(\"updateMockComponent\",[a.outerHTML],this.type,this.uuid)}onPropertyChanged(){this.refresh(this.$el)}}";
